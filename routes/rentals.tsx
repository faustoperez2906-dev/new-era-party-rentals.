...
import { Check, Music, Lightbulb, Snowflake, Coffee, Crown, Flame, Truck, ArrowUp } from "lucide-react";
        ...
        <div className="rounded-2xl border border-primary/30 bg-card/60 p-6 md:p-8">
          ...
          <ul className="mt-4 grid gap-3 text-sm text-muted-foreground md:grid-cols-2">
            <li className="flex items-start gap-2">
              ...
              {t("Delivery fee depends on how far your event is from us.", "La tarifa de entrega depende de la distancia de su evento.")}
            </li>
            <li className="flex items-start gap-2">
            ...
            <li className="flex items-start gap-2">
              ...
              {t("Pickup also has a charge based on distance from our location.", "La recolección también tiene un cargo según la distancia desde nuestra ubicación.")}
            </li>
            <li className="flex items-start gap-2">
              <Check className="w-4 h-4 text-primary mt-0.5 shrink-0" />
      ...
      <section className="mx-auto max-w-7xl px-5 md:px-8 pb-20 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {MACHINE_EXTRAS.map((m) => (
          <article key={m.name} className="rounded-2xl border border-border bg-card/40 p-6">
            ...
            <Link to="/quote" className="mt-4 inline-flex text-sm text-primary hover:underline">{t("Add to my quote →", "Agregar a mi cotización →")}</Link>
          </article>
        ))}
      </section>
